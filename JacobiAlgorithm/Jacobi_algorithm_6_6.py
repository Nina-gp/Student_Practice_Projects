...
"""Solves the equation Ax=b via the Jacobi iterative method for 6*6 coefficient matrix."""
...
def jacobi(A, b, error_treshold, X=None):
    ...
    if X is None:
        X = np.zeros(len(A[0])).reshape(6, 1)
...
print('Welcome to the Jacobi iterative equation solving program.\nThis program is designed according to the request for a matrix of six by six coefficients.\n')
while True:
